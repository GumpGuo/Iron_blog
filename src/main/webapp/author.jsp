        ...
        <article class="blog-main">
            ...
            <div class="am-g blog-content">
                <div class="am-u-lg-12">
                    <h3 class="am-article-title">作者：${authorName}</h3>
                    <ul>
                        <c:forEach var="blogType" items="${blogTypeList}">
                            <%--<li><a href="${pageContext.request.contextPath}/blog/articles/${blog.id}">${blog.title}</a></li>--%>
                            <li><a href="${pageContext.request.contextPath}/blog/author/${authorName}/${blogType.id}">${blogType.typeName} ${blogType.blogCount}</a></li>
                        </c:forEach>
                    </ul>
                </div>
            </div>
...
<%--<ul>--%>
    <%--<c:forEach var="blogType" items="${blogTypeList}">--%>
        <%--&lt;%&ndash;<li><a href="${pageContext.request.contextPath}/blog/articles/${blog.id}">${blog.title}</a></li>&ndash;%&gt;--%>
        <%--<li><a href="${pageContext.request.contextPath}/blog/author/${authorName}/${blogType.id}">${blogType.typeName} ${blogType.blogCount}</a></li>--%>
    <%--</c:forEach>--%>
<%--</ul>--%>

<%--<c:forEach var="nowPage" begin="1" end="${totalPage}">--%>
<%--<a href="${pageContext.request.contextPath}/blog/archive/${nowPage}?typeId=${typeId}">${nowPage}--</a>--%>
<%--</c:forEach>--%>
...
<%--<c:choose>--%>
    ...
    <%--<c:when test="${not empty author}">--%>
        <%--<c:forEach var="nowPage" begin="1" end="${totalPage}">--%>
            <%--<a href="${pageContext.request.contextPath}/blog/archive/${nowPage}?author=${author}">${nowPage}--</a>--%>
        <%--</c:forEach>--%>
    <%--</c:when>--%>
    <%--<c:otherwise>--%>
        <%--<c:forEach var="nowPage" begin="1" end="${totalPage}">--%>
            <%--<a href="${pageContext.request.contextPath}/blog/archive/${nowPage}">${nowPage}--</a>--%>
        <%--</c:forEach>--%>
    <%--</c:otherwise>--%>
<%--</c:choose>--%>
